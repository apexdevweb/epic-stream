...
if (isset($_POST['movie_add'])) {
    ...
    if (
        ...
        && !empty($_POST['movie_description']) && !empty($_FILES['movie_cover']) && !empty($_FILES['movie_cover']['name'])
    ) {
        ...
        $descript_of_vid = htmlspecialchars(strip_tags(nl2br($_POST['movie_description'])));
        $vid_id_cover = random_int(1, 1000);
        ...
        if ($movie_verif->rowCount() === 0) {
            ...
            if ($_FILES['movie_cover']['size'] <= $size_movie_cover) {
                ...
                if (in_array($movie_cover_upload_ext, $ext_movie_cover_valide)) {
                    $upload_route_cover = "assets/images/" . $vid_id_cover . "." . $movie_cover_upload_ext;
                    $movie_cover_transfert = move_uploaded_file($_FILES['movie_cover']['tmp_name'],  $upload_route_cover);
                    if ($movie_cover_transfert) {
                        try {
                            $insert_movie_cover = $bdd->prepare("INSERT INTO `film`(`vid_name`, `vid_description`, `vid_gender`, `vid_release_date`, `vid_cover`) VALUES (?,?,?,?,?)");
                            $insert_movie_cover->execute([$title_of_vid, $descript_of_vid, $gender_of_vid, $date_of_vid, $vid_id_cover]);
                        } catch (PDOException $e) {
